...
class OtherMediaController extends ShortPixelController
{
      ...
      protected function process_actions()
      {
        ...
        switch ($action)
        {
            ...
            break;
            case 'refresh':
                $this->shortPixel->refreshCustomFolders(true);
            break;
      ...
      {
          $this->view->rewriteHREF = '<script language="javascript"> history.pushState(null,null, "' . $_SERVER['HTTP_REFERER'] . '"); </script>';
      }
      ...
      protected function getPageURL($args = array())
      {
        ...
        return add_query_arg($page_args, $this->url);
    ...
    /*      if ($current != 1)
          {
            $parray[] = '<span class="previous"><a href="' . $this->getPageURL(array('paged' => ($current-1) )) . '"> ‹ </a></span>';
          }
          ...
          for($i = 1; $i <= $pages; $i++)
          {
              ...
              $parray[] = '<span class="' . $class . '"><a href="' . $this->getPageURL(array('paged' =>$i)) . '">' . $i . '</a></span>';
          }
